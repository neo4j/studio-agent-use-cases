// Patient Journey — query templates
//
...
//   - The shipped sample data holds 3,000 patients, 5,000 encounters, and 5,000 observations over a
//     fixed vocabulary of 8 conditions, 6 drugs, 4 providers, and 4 organisations. Aggregate/global
//     expected results below are for this seed and scale with the data; per-patient results for the
//     documented sample IDs are stable.
//
// Anchors are PARAMETERS, never hardcoded IDs. Before running an anchored query, either ask the
// user which entity they want or offer them a real anchor from query 0 (Available anchors).
// Do not invent IDs. To test in Neo4j Browser or cypher-shell, set the sample parameters first:
...
//
// Sections:
//   A. Point & tabular queries (1–7)
//   B. Graph / visualisation queries (G1–G5) — return nodes, relationships, and paths to render
...
// 0. Available anchors — the real IDs the agent can offer the user to try the templates.
...
// 2. Patient's prescribed drugs — every drug prescribed to one patient across their journey.
...
// 3. Patient's diagnosed conditions — all conditions diagnosed across one patient's journey.
...
// 5. Comorbidities associated with a condition — conditions co-occurring at the same encounter.
// Purpose: condition-centric aggregate for clinical decision support.
...
// 6. Provider workload — which patients a provider has seen and how often.
...
// These RETURN nodes, relationships, and paths so Neo4j Browser or Bloom draws
// a graph rather than a table. Keep viz result sets bounded with $limit.
// ============================================================================


// G1. Patient journey subgraph — the full ego network of one patient.
...
// G2. Shared-condition patient cluster — patients linked through one condition.
...
// G3. Comorbidity network — conditions co-diagnosed at the same encounter.
...
// G4. Care network around a condition — who treats it and where.
...
// G5. Drug co-prescription network — drugs prescribed together at the same encounter.
