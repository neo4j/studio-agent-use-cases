...
own. That layer is produced by resolving a variant rather than loaded with the
data, so it is not part of the entities and relationships set out below — it
appears once a variant has been resolved, and disappears when it is discarded.
...
`REQUIRES` carries the same two properties on all three of the pairs above.
`HAS_OPTION` carries none.
