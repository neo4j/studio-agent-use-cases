...
- **Declare only nodes and relationships that have a mapping.** Import reads the model as an import job definition and requires a source table, property columns, and endpoint ID columns for everything declared; an unmapped entity leaves the import definition in an error state the user cannot clear. A relationship or label a bundled query creates rather than loads therefore has no place in the model — document it in `SKILL.md` instead. See GRAPH_SPEC_FORMAT.md's "Declare only what Import can load".
