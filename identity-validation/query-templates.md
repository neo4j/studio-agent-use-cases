...
- `indexes.cypher` executed, for `Near-Duplicate SSN` (fulltext index) and optionally for spatial index support.
...
TODO(review): unproven query — none of the templates below have been executed against a live Neo4j. Each expected-result count was derived by reproducing the query's set semantics over the bundled CSVs, and each query was checked statically against `GRAPH_MODEL.json` for label, relationship-type, direction, and property correctness. Import the sample data into a disposable database and run all fourteen once to confirm, paying particular attention to the two APOC queries and the two fulltext fuzzy queries, whose scoring functions were modelled rather than executed.

## Placeholder & Invalid Identifier Detection

Run this first, before any matching query. Source systems often stuff a non-null filler value into a required field when the real value isn't known (`"none@none.com"`, `"N/A"`, `"999-999-9999"`). Those are not identifiers, and left alone they silently create false-positive matches between unrelated people. This flags known placeholder values with a `:Placeholder` label so the matching queries can exclude them.
...
`point.distance()` returns metres directly for geographic points. The query constructs the points inline from `latitude` and `longitude`, so it works against imported data with no extra step. If you materialise a `coordinates` point property (see `indexes.cypher`), substitute `l1.coordinates` and `l2.coordinates`.
...
//   by indexes.cypher; sample data imported; placeholder detection run.
// Expected: exactly 3 rows against the sample data — the seeded synthetic-identity
